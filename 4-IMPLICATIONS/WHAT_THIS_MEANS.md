...
## Claim Hygiene and Status Tags

This document uses the same status discipline as the technical corpus:
- **DERIVED**: obtained from the 5D derivation chain documented in the technical files.
- **FITTED / OBSERVATIONALLY SELECTED**: uses measurement input for selection or calibration.
- **OPEN**: unresolved or architecture-limited.

Canonical references for claim status and boundaries:
...
The framework proposes that irreversibility in 4D effective physics is generated by geometric structure in a 5D Kaluza-Klein formulation.

---

## Baseline Context

In standard thermodynamic treatments, macroscopic irreversibility is described statistically using low-entropy initial conditions and entropy growth.

This framework keeps those effective results but proposes a geometric parent explanation in 5D.
...
## 4D Effective Interpretation

In this model, the fifth dimension is compactified and integrated out in the 4D effective description. The resulting 4D system contains:
...
These are modeled as consequences of the reduced 5D geometry, not as standalone postulates in 4D.

---

## Quantitative Results and Status

Status labels below are aligned to [`SEPARATION.md`](../SEPARATION.md) and [`FALLIBILITY.md`](../FALLIBILITY.md).

### 1. The CMB Spectral Index: nₛ ≈ 0.9635 (**DERIVED with observational selection input**)
...
### 2. Cosmic Birefringence: β = 0.3513° (**DERIVED; canonical predictions are {≈0.273°, ≈0.331°}**)
...
### 3. The Nonminimal Coupling: α = φ₀⁻² (**DERIVED**)
...
### 4. Tensor-to-Scalar Ratio: r_braided ≈ 0.0315 < 0.036 ✓ (**DERIVED**)
...
### 5. The k_CS = 74 Topological Completeness Theorem (Pillar 74) (**DERIVED/STRUCTURAL with one measurement-linked constraint**)
...
TESTS:       63,639 passed · 23 skipped · 12 deselected · 0 failed
