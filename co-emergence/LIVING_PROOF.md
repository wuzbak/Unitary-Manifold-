...
The 8694 passing tests (main `tests/` suite) are not just quality assurance. In HILS terms, they are the
**defect function evaluation** at the current fixed point. And they are not the full picture:

```
defect ≈ 0  ↔  8694 tests passing (tests/) · 316 passing (recycling/) · 1234 passing (Unitary Pentad/) · 0 failures
```

Total: **10244 verified assertions across 129 test files. Zero failures.**
...
| **Verifiable** | `git log` shows the collaboration history; `pytest tests/ recycling/ "Unitary Pentad/" -q` verifies 10244 assertions in under 150 seconds; authorship attributions are explicit |
