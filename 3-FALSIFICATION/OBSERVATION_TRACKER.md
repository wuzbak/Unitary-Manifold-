...
| **P8** | Lab-scale CP asymmetry in certified (5,7) condensed matter | \(A_{CP}^{lab}=(\Gamma_+-\Gamma_-)/(\Gamma_++\Gamma_-)\) | **Order \(10^{-5}\)** (geometry-transfer target from \(J_{geo}\)) | JJ/SQUID arrays; topological-insulator winding devices | Now | 🟡 PENDING — no decision-grade \(\sigma_A\le10^{-5}\) campaign logged yet | 2026-05-09 | Launch paired-conjugate protocol campaign with topology-swap and sign-reversal controls; falsify if null at \(10^{-5}\) sensitivity with certified topology |
