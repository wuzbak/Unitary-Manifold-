...
%  Version 9.28 (post cross-disciplinary peer review — see Outstanding Gaps section)
...
{\large Version 9.28 (post peer review; see §Outstanding Gaps)}}
...
% ============================================================
\section*{Outstanding Gaps and Honest Status (v9.28)}
...
\begin{enumerate}
  \item \textbf{ADM time parameterization:} \texttt{evolution.py} uses a Ricci-flow
        parameter, not coordinate time $x^0$.  Pillar~41 provides a partial correction
        $\Omega(\varphi) = 1/\varphi$; a full ADM 3+1 decomposition is open.
  \item \textbf{$n_w = 5$ uniqueness residual:} The $\eta$-invariant class requirement
        ($\bar{\eta} \equiv \tfrac{1}{2} \bmod 1$ at $Z_2$-odd fixed planes) is
        physically motivated but not yet proved as a theorem.  Planck $n_s$ provides
        independent $4\sigma$ confirmation.
  \item \textbf{$\mathrm{SU}(3) \times \mathrm{SU}(2)$:} Not produced from 5D geometry
        (Witten 1981 — minimum 11D required).
