...
- Sustained head-to-head replacement gating is available through `evaluateMerlinEmpiricalGate`, `runMerlinStageAReceipts`, and `getMerlinReplacementReadiness`, with `/api/merlin/promotion-packet` now serving the concrete readiness packet rather than an evidence-empty placeholder.
