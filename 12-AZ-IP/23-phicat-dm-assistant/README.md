# PhiCat DM Guide & Player Assistant

## Product 23 of the AxiomZero suite

PhiCat DM Guide & Player Assistant is an offline-first Dungeons & Dragons **5e / 5.5e** campaign system built to give Dungeon Masters and players one coherent surface for campaign tracking, character building, invite-code joins, encounter planning, grounded image prompting, and rules-aware PhiCat support.
...
- **PhiCat expert assistant** for table-ready summaries, rules-digest notes, and next-step prompts.
...
- `POST /api/campaigns/<id>/phicat` (legacy alias: `/api/campaigns/<id>/merlin`)
- `POST /api/player-dashboard/standalone`

## Quick start

```bash
cd 12-AZ-IP/23-phicat-dm-assistant
...
- **PhiCat role:** PhiCat is embedded as the domain expert and returns rules-aware guidance, not just a generic chatbot answer.
