 ...
 *
 * Kotlin class: com.gibbernode.audio.GGWaveNative  (object with external funs)
 *
 * Threading contract:
...
Java_com_gibbernode_audio_GGWaveNative_nativeInit(
    JNIEnv* /*env*/,
    jobject /*thiz*/,
    jint    sampleRate,
    jint    payloadLength)
{
    ...
    // Audible protocols only — ultrasound off by default (BV9900 Pro is waterproofed)
...
Java_com_gibbernode_audio_GGWaveNative_nativeFree(
    JNIEnv* /*env*/,
    jobject /*thiz*/,
    jlong   handle)
{
...
Java_com_gibbernode_audio_GGWaveNative_nativeEncode(
    JNIEnv* env,
    jobject /*thiz*/,
    jlong   handle,
    jstring payload,
...
Java_com_gibbernode_audio_GGWaveNative_nativeDecode(
    JNIEnv* env,
    jobject /*thiz*/,
    jlong       handle,
    jshortArray samples)
{
    ...
    if (result <= 0) {
        // 0 = no message yet; negative = error.  Both are silent at LOGI level.
        return nullptr;
    }

    LOGI("nativeDecode OK  payload_len=%d", result);
    // result is the number of bytes written to outputBuf.
