...
**Laws are more compressed than the phenomena they govern.** The smallest seed contains the largest explanation. n_w = 5 — one integer, ~3 bits — encodes 590+ pillars of physics, ~49,850 passing tests, and the conditions for the minds that discovered it.
...
Together, they bracket the project. Omega is the summit. Holon Zero is the foundation. The project lives between them — 590+ pillars of physics, ~49,850 tests, zero failures, and one loop that closes.
...
This canonical folder preserves the richer `holon_zero/` package (including `subpillars/`) while also carrying forward the hyphenated-documentation lineage from `holon-zero/`.
