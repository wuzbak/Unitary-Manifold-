# PsiCat
## Product 20 — PsiCat Navigator (Quantum Cat interface; legacy OX-compatible)

> "Ask the repository. Keep the gates visible. Keep the governance boundary explicit. PsiCat is the application OX was supposed to become."

- **Folder:** `12-AZ-IP/20-psicat-navigator/`
- **Version:** `v23.2`
...
- **API endpoints:** `/api/psicat`, `/api/psicat/status`, `/api/psicat/identity`, `/api/psicat/policy`, `/api/psicat/runtime`, `/api/psicat/program`, `/api/psicat/program-office`, `/api/psicat/control-tower`, `/api/psicat/sync-checks`, `/api/psicat/benchmarks`, `/api/psicat/testing-stack`, `/api/psicat/training-architecture`, `/api/psicat/arc-agi`, `/api/psicat/training-dataset`, `/api/psicat/training-curation`, `/api/psicat/context-scaffold`, `/api/psicat/ast-context-records`, `/api/psicat/mlflow-manifests`, `/api/psicat/open-science-registry`, `/api/psicat/open-weight-acquisition`, `/api/psicat/training-framework-stack`, `/api/psicat/competitive-benchmarks`, `/api/psicat/dual-lane-master-sprint`, `/api/psicat/three-lane-intensive-sprint`, `/api/psicat/continuous-learning`, `/api/psicat/performance-lane`, `/api/psicat/training-execution-queue`, `/api/psicat/training-execution-bundle`, `/api/psicat/lane-e-runtime-profiles`, `/api/psicat/lane-progress-ledgers`, `/api/psicat/training-cycle`, `/api/psicat/performance-gate-evaluate`, `/api/psicat/training-challenge-pack`, `/api/psicat/benchmark-corpora`, `/api/psicat/stage-a-receipts`, `/api/psicat/stage-b-receipts`, `/api/psicat/stage-c-receipts`, `/api/psicat/stage-d-receipts`, `/api/psicat/stage-e-receipts`, `/api/psicat/replacement-readiness`, `/api/psicat/frontier-readiness`, `/api/psicat/review-packet`, `/api/psicat/targeted-rigor-sprint`, `/api/psicat/spc-phase0-packet`, `/api/psicat/spc-phase1-baseline`, `/api/psicat/spc-phase2-applied-pressure`, `/api/psicat/spc-phase3-live-readiness`, `/api/psicat/achievement-benchmark-promotion-sprint`, `/api/psicat/heavy-lane`, `/api/psicat/model-board`, `/api/psicat/hardware-board`, `/api/psicat/kernel-runtime`, `/api/psicat/kernel-receipts`, `/api/psicat/kernel-benchmarks`, `/api/psicat/kernel-gate`, `/api/psicat/kernel-risk`, `/api/psicat/kernel-escalation`, `/api/psicat/kernel-governance`, `/api/psicat/kernel-batch-plan`, `/api/psicat/compactification-sanity`, `/api/psicat/compactification-ingest`, `/api/psicat/topology-adjacent`, `/api/psicat/execution-board`, `/api/psicat/convergence-charter`, `/api/psicat/masterclass-execution`, `/api/psicat/swarm-observatory`, `/api/psicat/branch-convergence`, `/api/psicat/branch-convergence-review`, `/api/psicat/swarm-analyze`, `/api/psicat/validation-resilience`, `/api/psicat/benchmark-artifacts`, `/api/psicat/training-artifacts`, `/api/psicat/promotion-packet`, `/api/psicat/local-execution/status`, `/api/psicat/local-execution/run`, `/api/agentToolkit`, `/api/agentInvoke`, `/api/agentOrchestrate`
...
6. [`PSICAT_THREE_LANE_INTENSIVE_SPRINT.md`](./PSICAT_THREE_LANE_INTENSIVE_SPRINT.md) (apps/tools + books/articles + adversarial growth + continuous learning)
7. [`PSICAT_SPC_EXPERT_ACCELERATION_MASTER_PLAN.md`](./PSICAT_SPC_EXPERT_ACCELERATION_MASTER_PLAN.md) (immediate three-phase parallel expert uplift for SPC operations)
8. [`PSICAT_SPC_BENCHMARK_GATES.md`](./PSICAT_SPC_BENCHMARK_GATES.md) (hard fail conditions + promotion gates + evidence packet requirements)
9. [`training/training_execution/psicat_spc_phase0_execution_packet.json`](./training/training_execution/psicat_spc_phase0_execution_packet.json) (machine-readable immediate action board)
10. [`training/training_execution/psicat_spc_phase1_baseline_receipts.json`](./training/training_execution/psicat_spc_phase1_baseline_receipts.json) (first baseline batteries with hold/clear/demote ledger and gate verdict)
11. [`training/training_execution/psicat_spc_phase1_baseline_diff.json`](./training/training_execution/psicat_spc_phase1_baseline_diff.json) (delta report vs prior baseline run)
12. [`PSICAT_PROGRAM.md`](./PSICAT_PROGRAM.md) (implemented surfaces + ledger)
...
- PsiCat Navigator is the canonical product label; legacy OX naming/endpoints remain for compatibility.
...
## How PsiCat differs from the main Oracle

- The Oracle is the capstone synthesis engine for arbitrary human systems.
- PsiCat is repository-specific and physics-navigation oriented.
- The Oracle computes system analyses; PsiCat answers questions about pillars, tests, gates, falsifiers, and linked AxiomZero tools.
- The Oracle foregrounds Pentad synthesis; PsiCat foregrounds traceable retrieval-style navigation plus structured citations.
- PsiCat remains narrower than the Oracle, but broader than the old OX shell.
...
- OpenRouter is not the primary path; PsiCat defaults to sovereign local/offline-first behavior.
- OpenRouter compat only activates when router confidence is low and compatibility mode is explicitly enabled.

## PsiCat response contract
...
- Every response is normalized to:
  ...
  - `FOLLOWUPS:`
  - `Sources:`
- Every `/api/psicat` response now also returns:
...
- `GET /api/psicat/context-scaffold` exposes the typed architectural scaffold PsiCat assembles before a live answer.
- `GET /api/psicat/memory` exposes multi-tier memory state, contradictions, and recall audits.
- `GET /api/psicat/memory-geometry` exposes geometry-constrained memory landmarks (`hyperbolic_tree`, `riemannian_focus`, `topological_persistence`) for long-horizon recall diagnostics.
- `GET /api/psicat/telemetry` exposes recent run summaries for measurement and rollout gating.
- `GET /api/psicat/status` now returns `memory_profile_token`; cross-device resume via `X-PsiCat-Profile-Token` also requires `X-PsiCat-Profile-Key` matching server-side `MERLIN_PROFILE_SHARED_KEY`.
- `GET /api/psicat/program-office` exposes command authority, decision/risk ledgers, and squad ownership for replacement governance.
- `GET /api/psicat/control-tower` exposes live replacement readiness, drift alerts, trendlines, and fail-closed deployment eligibility.
- `GET /api/psicat/convergence-charter` exposes the approved execution-spine charter, completion maps, phases, and sanity rules for monorepo convergence work.
...
- `GET /api/psicat/arc-agi` exposes the ARC-AGI shadow-training, contamination-control, and benchmark-gating contract for PsiCat.
...
- `GET /api/psicat/training-dataset` accepts optional `include_ast_context=true` and `ast_file_limit=<n>` to inject deterministic AST-density and tool-definition training records.
...
- `GET /api/psicat/training-curation` accepts optional `include_ast_context=true` and `ast_file_limit=<n>` for AST-enriched curation cycles.
- `GET /api/psicat/ast-context-records` returns deterministic AST-context and tool-definition training records for direct inspection/export.
...
- `GET /api/psicat/performance-lane` exposes the dedicated throughput-optimization lane with hard speed gates, profiler-first workflow, ROI execution order, and local-first sovereignty constraints.
- `POST /api/psicat/performance-gate-evaluate` evaluates baseline vs candidate training receipts and returns pass/hold against Lane E throughput and regression gates.
...
- `GET /api/psicat/training-execution-bundle` exposes the retained training execution bundle directly, including lane progress, challenge pack, execution-cycle snapshot, and Lane E runtime profile evidence payload.
- `GET /api/psicat/training-execution-bundle` accepts optional `refresh_lane_e_profiles=true` to force fresh Lane E runtime profile recapture in that bundle.
- `GET /api/psicat/lane-e-runtime-profiles` exposes Lane E runtime profile artifact metadata/payload directly, with optional `refresh` recapture mode (`true/false`).
...
- `GET /api/psicat/training-artifacts` accepts optional `refresh_lane_e_profiles=true` to force fresh Lane E runtime profile recapture in the embedded execution-bundle preview.
- `GET /api/psicat/training-artifacts` accepts optional `include_ast_context=true` and `ast_file_limit=<n>` to include AST-context density records inside the exported training dataset bundle.
...
- `GET /api/psicat/stage-b-receipts` runs the self-hosted Stage B receipt set and returns comparable PsiCat/incumbent runs.
- `GET /api/psicat/stage-c-receipts` runs the self-hosted Stage C receipt set and returns comparable PsiCat/incumbent runs.
...
- `GET /api/psicat/spc-phase2-applied-pressure` translates the phase-1 lane evidence into supervised applied-pressure gates using the phase-0 thresholds for decision quality, false confidence, and traceability while preserving observatory incidents as blockers.
- `GET /api/psicat/spc-phase3-live-readiness` converts the phase-2 lane receipts into consecutive integrated-run checks and a governance traceability review before any live-readiness language advances.
...
- `GET /api/psicat/kernel-batch-plan` exposes bounded chunk planning metadata for large kernel requests before execution.
...
## PsiCat replacement program implementation

- The implementation ledger for the 13-point PsiCat replacement plan lives in `PSICAT_PROGRAM.md`.
...
- Browser and AI-evaluation doctrine now ships through `ox_navigator/engine/merlin_testing_stack.py`, `testing/psicat_prompt_contracts.json`, and `/api/psicat/testing-stack`.

## Stage A benchmark and measurement layer

- PsiCat now exposes a first-class Stage A benchmark corpus for parity capture before wider takeover.
- Each PsiCat run records latency, estimated tokens, estimated cost, estimated energy, routing lane, provider, provenance coverage, and memory/contradiction signals.
- Benchmark evaluation can score a response against PsiCat benchmark corpora using `evaluatePsiCatBenchmarkResponse`, with explicit stage selection available for Stage A/B/C scoring.
- Sustained head-to-head replacement gating is available through `evaluatePsiCatEmpiricalGate`, `runPsiCatStageAReceipts`, `getPsiCatReplacementReadiness`, and `getPsiCatStageAArtifacts`, while `/api/psicat/promotion-packet` remains the legacy compatibility view.
- `GET /api/psicat/control-tower` now includes mentorship-to-runtime closure checks with fail-closed completion logic and explicit evidence-required fields.
- CI artifact export: `python tools/export_merlin_stage_a_artifacts.py --limit 3 --output /tmp/merlin-stage-a-artifacts.json`.
- Training artifact export: `python tools/export_merlin_training_artifacts.py` (default output: `training/training_artifacts/training_artifacts.json`; optional `--refresh-lane-e-profiles` forces fresh Lane E runtime profile recapture in the embedded execution preview; optional `--include-ast-context --ast-file-limit <n>` injects deterministic AST-density training records).
- JSONL dataset export: `python tools/export_merlin_training_jsonl.py` (default output: `training/training_jsonl/`).
- JSONL dataset export supports deterministic AST enrichment via `--include-ast-context --ast-file-limit <n>`.
...
cd 12-AZ-IP/20-psicat-navigator
...
- **Tests fail unexpectedly:** run `python -m pytest tests/` from inside `12-AZ-IP/20-psicat-navigator/`.
