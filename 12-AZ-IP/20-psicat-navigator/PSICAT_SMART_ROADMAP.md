...
- `PSICAT_FRONTIER_ROADMAP.md` (full operating plan)
- `PSICAT_PROGRAM.md` (implementation ledger and API surface)
...
1. Start here ([`PSICAT_SMART_ROADMAP.md`](./PSICAT_SMART_ROADMAP.md))
2. Read full strategy ([`PSICAT_FRONTIER_ROADMAP.md`](./PSICAT_FRONTIER_ROADMAP.md))
3. Inspect implementation surfaces ([`PSICAT_PROGRAM.md`](./PSICAT_PROGRAM.md))
4. Verify runtime/API behavior ([`README.md`](./README.md))
5. Run receipts from the Merlin folder (uses the Merlin-local `requirements.txt` in that folder): `cd 12-AZ-IP/20-psicat-navigator && pip install -r requirements.txt && python ./tools/run_merlin_stage_a_benchmarks.py --json`
