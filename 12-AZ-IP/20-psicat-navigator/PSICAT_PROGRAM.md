...
- `/api/merlin/training-execution-queue`, `/api/merlin/lane-progress-ledgers`, and `/api/merlin/training-cycle` now expose live queue state, per-lane progress ledgers, and auditable retained training work, including proof-foundry reviewer packets, bridge-audit surfaces, and performance-gate receipts.
