...
`getMerlinTrainingFrameworkStack()` now exposes a governed framework matrix grouped by distributed scale (DeepSpeed, Megatron-LM, TorchTitan), fine-tuning/alignment (Transformers, PEFT, TRL, bitsandbytes, LitGPT), and mechanistic learning (nanoGPT, llm.c), with explicit fail-closed promotion gates and branch-collision integration controls for active parallel development.
...
- PsiCat RAG now assembles a typed context scaffold before generation rather than relying on raw retrieval alone. The scaffold carries lane classification, dominant gate/boundary guardrails, retrieval hits, AST structure hints, contradiction-ledger state, and runtime/tool alignment surfaces.
- `GET /api/psicat/context-scaffold`, `getMerlinContextScaffold`, and `/api/assistant` context scaffolds now expose the same governed preflight packet so both the product shell and the lightweight bot can inject structural context without flooding prompts with raw corpus dumps.
