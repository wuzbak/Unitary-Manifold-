...
- `PHICAT_FRONTIER_ROADMAP.md` (full operating plan)
- `PHICAT_PROGRAM.md` (implementation ledger and API surface)
...
1. Start here ([`PHICAT_SMART_ROADMAP.md`](./PHICAT_SMART_ROADMAP.md))
2. Read full strategy ([`PHICAT_FRONTIER_ROADMAP.md`](./PHICAT_FRONTIER_ROADMAP.md))
3. Inspect implementation surfaces ([`PHICAT_PROGRAM.md`](./PHICAT_PROGRAM.md))
4. Verify runtime/API behavior ([`README.md`](./README.md))
5. Run receipts from the Merlin folder (uses the Merlin-local `requirements.txt` in that folder): `cd 12-AZ-IP/20-phicat-navigator && pip install -r requirements.txt && python ./tools/run_merlin_stage_a_benchmarks.py --json`
