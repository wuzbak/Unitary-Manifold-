...
MERLIN_THREE_LANE_DOC = PRODUCT_ROOT / "PHICAT_THREE_LANE_INTENSIVE_SPRINT.md"
MERLIN_EXECUTION_BOARD_DOC = PRODUCT_ROOT / "PHICAT_EXECUTION_BOARD.md"
MERLIN_VALIDATION_RESILIENCE_DOC = PRODUCT_ROOT / "PHICAT_VALIDATION_RESILIENCE_PACKET.md"
    ...
    return {
        "library": {
            "curated_canonical_sources": [
                ...
                "12-AZ-IP/20-phicat-navigator/README.md",
            ],
...
def get_current_stack_baseline() -> dict[str, Any]:
    return {
        "baseline_product": "12-AZ-IP/20-phicat-navigator",
...
def get_trust_source_library() -> dict[str, Any]:
    return {
        "policy": (
            "Authoritative-first source hierarchy for PhiCat expert tracks. "
...
def run_sync_checks() -> dict[str, Any]:
    ...
    for route in list(parsed_routes):
        if route.startswith("/api/phicat"):
            expanded_routes.add("/api/merlin" + route[len("/api/phicat"):])
    ...
    for endpoint in list(readme_endpoints):
        if endpoint.startswith("/api/phicat"):
            expanded_readme_endpoints.add("/api/merlin" + endpoint[len("/api/phicat"):])
    ...
    return {
        ...
        "codeql_scope_reduction_strategy": {
            ...
            "first_candidate_paths": [
                "12-AZ-IP/20-phicat-navigator/ox_navigator/",
                "12-AZ-IP/20-phicat-navigator/tests/",
                "TOOLS/checks/copilot_review_orchestrator.py",
            ],
            "do_not_claim": [
    ...
    *,
    ...
    def _mlflow_runner_command(experiment: str, output: str) -> str:
        return _shell_command(
            python_executable,
            "12-AZ-IP/20-phicat-navigator/tools/run_merlin_mlflow_experiment.py",
            "--experiment",
            experiment,
            "--limit",
            str(resolved_limit),
