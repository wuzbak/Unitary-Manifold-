...
class OxRequestHandler(SimpleHTTPRequestHandler):
    ...
    def _json(self, payload: dict, status: int = 200) -> None:
        ...
        self.send_header('X-PhiCat-Session-Persistence', 'process_local_memory')
        ...
        if pending_cookie:
            ...
            self.send_header('Set-Cookie', f'phicat_profile_id={_sign_session_id(pending_cookie)}; Path=/; HttpOnly; SameSite=Lax{secure}')
    ...
    def _validate_handshake(
        ...
        *,
        ...
        with _MERLIN_HANDSHAKE_LOCK:
            self._prune_handshake_state()
            header_challenge = str(self.headers.get("X-PhiCat-Handshake-Challenge") or self.headers.get("X-Merlin-Handshake-Challenge") or "").strip()
            body_challenge = str((payload or {}).get("phicat_handshake_challenge") or (payload or {}).get("merlin_handshake_challenge") or "").strip()
            query_challenge = str((params or {}).get("phicat_handshake_challenge", [""])[0] or (params or {}).get("merlin_handshake_challenge", [""])[0] or "").strip()
            ...
            header_proof = str(self.headers.get("X-PhiCat-Handshake-Proof") or self.headers.get("X-Merlin-Handshake-Proof") or "").strip()
            header_receipt = str(self.headers.get("X-PhiCat-Handshake-Receipt") or self.headers.get("X-Merlin-Handshake-Receipt") or "").strip()
            body_proof = str((payload or {}).get("phicat_handshake_proof") or (payload or {}).get("merlin_handshake_proof") or "").strip()
            body_receipt = str((payload or {}).get("phicat_handshake_receipt") or (payload or {}).get("merlin_handshake_receipt") or "").strip()
            query_proof = str((params or {}).get("phicat_handshake_proof", [""])[0] or (params or {}).get("merlin_handshake_proof", [""])[0] or "").strip()
            query_receipt = str((params or {}).get("phicat_handshake_receipt", [""])[0] or (params or {}).get("merlin_handshake_receipt", [""])[0] or "").strip()
            proof = body_proof or header_proof or query_proof
            receipt = body_receipt or header_receipt or query_receipt
            token_query = str((params or {}).get("phicat_profile_token", [""])[0] or (params or {}).get("merlin_profile_token", [""])[0] or "").strip()
            token_header = str(self.headers.get("X-PhiCat-Profile-Token") or self.headers.get("X-Merlin-Profile-Token") or "").strip()
            token_body = str((payload or {}).get("phicat_handshake_profile_token") or (payload or {}).get("merlin_handshake_profile_token") or "").strip()
            if not token_body:
    ...
    def _profile_hint(self, *, payload: dict | None = None, params: dict | None = None) -> str:
        query_profile = str((params or {}).get("phicat_profile_token", [""])[0] or (params or {}).get("merlin_profile_token", [""])[0] or "").strip()
        header_profile = str(self.headers.get("X-PhiCat-Profile-Token") or self.headers.get("X-Merlin-Profile-Token") or "").strip()
        ...
        key_header = str(self.headers.get("X-PhiCat-Profile-Key") or self.headers.get("X-Merlin-Profile-Key") or "").strip()
    ...
    def _merlin_session(self, *, profile_hint: str = "") -> tuple[str, MerlinSession, threading.RLock]:
        ...
        for part in raw_cookie.split(';'):
            ...
            if key in {'phicat_profile_id', 'merlin_profile_id'} and value.strip():
    ...
    def do_GET(self):  # noqa: N802
        ...
        if route_path.startswith('/api/merlin'):
            route_path = '/api/phicat' + route_path[len('/api/merlin'):]
        ...
        if parsed.path.startswith('/api/phicat') or parsed.path.startswith('/api/merlin') or parsed.path.startswith('/api/ox'):
        ...
        with merlin_lock:
            if route_path == '/api/phicat/status':
                ...
                'rebrand_label': 'REBRAND-2026-09-PHICAT',
                'model': MODEL_ID,
                ...
                'compatibility': {
                    'canonical_query_endpoint': '/api/phicat',
                ...
                return
            if route_path == '/api/phicat/program':
                self._json({'ok': True, 'program': get_full_program_blueprint()})
                self._persist_session(session_id, merlin_session)
                return
            if route_path == '/api/phicat/program-office':
                self._json({'ok': True, 'program_office': get_program_office()})
                self._persist_session(session_id, merlin_session)
                return
            if route_path == '/api/phicat/control-tower':
                ...
                self._persist_session(session_id, merlin_session)
                return
            if route_path == '/api/phicat/memory':
                self._json({'ok': True, 'memory': merlin_session.get_public_memory_state()})
                self._persist_session(session_id, merlin_session)
                return
            if route_path == '/api/phicat/memory-geometry':
            ...
            if route_path == '/api/phicat/inference/health':
                ...
                self._persist_session(session_id, merlin_session)
                return
            if route_path == '/api/phicat/reasoning-chain':
                ...
                self._persist_session(session_id, merlin_session)
                return
            if route_path == '/api/phicat/counterexample-digest':
                ...
                self._persist_session(session_id, merlin_session)
                return
            if route_path == '/api/phicat/energy-ledger':
                limit, error = _parse_positive_int_query_param(params, 'limit', 10)
                if error:
                    self._json({'ok': False, 'error': error}, status=400)
                    return
                ...
                self._persist_session(session_id, merlin_session)
                return
            if route_path == '/api/phicat/runtime':
                self._json({
                'ok': True,
                'runtime': {
                ...
                return
            if route_path == '/api/phicat/benchmarks':
                self._json({
                'ok': True,
                ...
                self._persist_session(session_id, merlin_session)
                return
            if route_path == '/api/phicat/training-architecture':
                ...
                self._persist_session(session_id, merlin_session)
                return
            if route_path == '/api/phicat/training-dataset':
                limit, error = _parse_int_query_param(params, 'limit', 12)
                if error:
                    self._json({'ok': False, 'error': error}, status=400)
                    return
                ...
                self._persist_session(session_id, merlin_session)
                return
            if route_path == '/api/phicat/training-curation':
                limit, error = _parse_int_query_param(params, 'limit', 12)
                if error:
                    self._json({'ok': False, 'error': error}, status=400)
                    return
                ...
                self._persist_session(session_id, merlin_session)
                return
            if route_path == '/api/phicat/domain-benchmark-corpus':
                self._json({
                'ok': True,
                ...
                self._persist_session(session_id, merlin_session)
                return
            if route_path == '/api/phicat/domain-gate-contract':
                status, payload = _tool_data_or_error(route_tool(
                ...
                self._persist_session(session_id, merlin_session)
                return
            if route_path == '/api/phicat/domain-receipts':
                ...
                self._persist_session(session_id, merlin_session)
                return
            if route_path == '/api/phicat/open-science-registry':
                self._json({
                'ok': True,
                ...
                self._persist_session(session_id, merlin_session)
                return
            if route_path == '/api/phicat/open-weight-acquisition':
                self._json({
                'ok': True,
                ...
                self._persist_session(session_id, merlin_session)
                return
            if route_path == '/api/phicat/dual-lane-master-sprint':
                self._json({
                'ok': True,
                ...
                self._persist_session(session_id, merlin_session)
                return
            if route_path == '/api/phicat/three-lane-intensive-sprint':
                ...
                self._persist_session(session_id, merlin_session)
                return
            if route_path == '/api/phicat/continuous-learning':
                limit, error = _parse_int_query_param(params, 'limit', 24)
                if error:
                    self._json({'ok': False, 'error': error}, status=400)
                    return
                ...
                self._persist_session(session_id, merlin_session)
                return
            if route_path == '/api/phicat/training-execution-queue':
                limit, error = _parse_int_query_param(params, 'limit', 24)
                if error:
                    self._json({'ok': False, 'error': error}, status=400)
                    return
                ...
                self._persist_session(session_id, merlin_session)
                return
            if route_path == '/api/phicat/lane-progress-ledgers':
                ...
                self._persist_session(session_id, merlin_session)
                return
            if route_path == '/api/phicat/training-challenge-pack':
                limit, error = _parse_positive_int_query_param(params, 'limit', 12)
                if error:
                    self._json({'ok': False, 'error': error}, status=400)
                    return
                ...
                self._persist_session(session_id, merlin_session)
                return
            if route_path == '/api/phicat/trust-source-library':
                self._json({
                'ok': True,
                ...
                self._persist_session(session_id, merlin_session)
                return
            if route_path == '/api/phicat/knowledge-unknowns':
                self._json({
                'ok': True,
                ...
                self._persist_session(session_id, merlin_session)
                return
            if route_path == '/api/phicat/regulatory-change-watch':
                self._json({
                'ok': True,
                ...
                self._persist_session(session_id, merlin_session)
                return
            if route_path == '/api/phicat/domain-research-missions':
                self._json({
                'ok': True,
                ...
                self._persist_session(session_id, merlin_session)
                return
            if route_path == '/api/phicat/expert-mastery-program':
                self._json({
                'ok': True,
                ...
                self._persist_session(session_id, merlin_session)
                return
            if route_path == '/api/phicat/mlflow-manifests':
                limit, error = _parse_int_query_param(params, 'limit', 12)
                if error:
                    self._json({'ok': False, 'error': error}, status=400)
                    return
                ...
                self._persist_session(session_id, merlin_session)
                return
            if route_path == '/api/phicat/competitive-benchmarks':
                self._json({
                'ok': True,
                ...
                self._persist_session(session_id, merlin_session)
                return
            if route_path == '/api/phicat/benchmark-corpora':
                ...
                self._persist_session(session_id, merlin_session)
                return
            if route_path == '/api/phicat/stage-a-receipts':
                ...
                self._persist_session(session_id, merlin_session)
                return
            if route_path == '/api/phicat/stage-b-receipts':
                limit, error = _parse_int_query_param(params, 'limit', 3)
                if error:
                    self._json({'ok': False, 'error': error}, status=400)
                    return
                ...
                self._persist_session(session_id, merlin_session)
                return
            if route_path == '/api/phicat/stage-c-receipts':
                limit, error = _parse_int_query_param(params, 'limit', 3)
                if error:
                    self._json({'ok': False, 'error': error}, status=400)
                    return
                ...
                self._persist_session(session_id, merlin_session)
                return
            if route_path == '/api/phicat/stage-d-receipts':
                limit, error = _parse_int_query_param(params, 'limit', 3)
                if error:
                    self._json({'ok': False, 'error': error}, status=400)
                    return
                ...
                self._persist_session(session_id, merlin_session)
                return
            if route_path == '/api/phicat/stage-e-receipts':
                limit, error = _parse_int_query_param(params, 'limit', 3)
                if error:
                    self._json({'ok': False, 'error': error}, status=400)
                    return
                ...
                self._persist_session(session_id, merlin_session)
                return
            if route_path == '/api/phicat/replacement-readiness':
                limit, error = _parse_int_query_param(params, 'limit', 3)
                if error:
                    self._json({'ok': False, 'error': error}, status=400)
                    return
                ...
                self._persist_session(session_id, merlin_session)
                return
            if route_path == '/api/phicat/frontier-readiness':
                limit, error = _parse_int_query_param(params, 'limit', 3)
                if error:
                    self._json({'ok': False, 'error': error}, status=400)
                    return
                ...
                self._persist_session(session_id, merlin_session)
                return
            if route_path == '/api/phicat/review-packet':
                ...
                self._persist_session(session_id, merlin_session)
                return
            if route_path == '/api/phicat/heavy-lane':
                limit, error = _parse_int_query_param(params, 'limit', 3)
                if error:
                    self._json({'ok': False, 'error': error}, status=400)
                    return
                ...
                self._persist_session(session_id, merlin_session)
                return
            if route_path == '/api/phicat/model-board':
                self._json({
                'ok': True,
                ...
                self._persist_session(session_id, merlin_session)
                return
            if route_path == '/api/phicat/execution-board':
                limit, error = _parse_int_query_param(params, 'limit', 2)
                if error:
                    self._json({'ok': False, 'error': error}, status=400)
                    return
                ...
                self._persist_session(session_id, merlin_session)
                return
            if route_path == '/api/phicat/validation-resilience':
                limit, error = _parse_int_query_param(params, 'limit', 5)
                if error:
                    self._json({'ok': False, 'error': error}, status=400)
                    return
                ...
                self._persist_session(session_id, merlin_session)
                return
            if route_path == '/api/phicat/benchmark-artifacts':
                limit, error = _parse_int_query_param(params, 'limit', 3)
                if error:
                    self._json({'ok': False, 'error': error}, status=400)
                    return
                ...
                self._persist_session(session_id, merlin_session)
                return
            if route_path == '/api/phicat/training-artifacts':
                limit, error = _parse_int_query_param(params, 'limit', 12)
                if error:
                    self._json({'ok': False, 'error': error}, status=400)
                    return
                ...
                self._persist_session(session_id, merlin_session)
                return
            if route_path == '/api/phicat/promotion-packet':
                status, payload = _tool_data_or_error(route_tool(
                ...
                self._persist_session(session_id, merlin_session)
                return
            if route_path == '/api/phicat/identity':
                self._json({'ok': True, 'identity': get_identity_policy()})
                self._persist_session(session_id, merlin_session)
                return
            if route_path == '/api/phicat/policy':
                self._json({
                'ok': True,
                'policy': {
                ...
                self._persist_session(session_id, merlin_session)
                return
            if route_path == '/api/phicat/sync-checks':
    ...
    def do_POST(self):  # noqa: N802
        ...
        if route_path in ('/api/phicat', '/api/ox') and str(getattr(self, '_profile_token_state', '')) in {'invalid_token_signature', 'invalid_shared_key', 'shared_key_not_configured'}:
        ...
        handshake_material_present = any([
            str(self.headers.get("X-PhiCat-Handshake-Challenge") or self.headers.get("X-Merlin-Handshake-Challenge") or "").strip(),
            str(self.headers.get("X-PhiCat-Handshake-Proof") or self.headers.get("X-Merlin-Handshake-Proof") or "").strip(),
            str(self.headers.get("X-PhiCat-Handshake-Receipt") or self.headers.get("X-Merlin-Handshake-Receipt") or "").strip(),
        ...
        requires_handshake = route_path in ('/api/phicat', '/api/ox')
        ...
        with merlin_lock:
            try:
                ...
                if route_path in ('/api/phicat', '/api/ox'):
                    ...
                    self._json(result)
                    return
                if route_path == '/api/phicat/research-cycle':
                    ...
                    return
                if route_path == '/api/phicat/training-cycle':
