# PhiCat
## Product 20 — PhiCat Navigator (Quantum Cat interface; legacy OX-compatible)

> "Ask the repository. Keep the gates visible. Keep the governance boundary explicit. PhiCat is the application OX was supposed to become."

- **Folder:** `12-AZ-IP/20-phicat-navigator/`
- **Version:** `v23.2`
...
- **API endpoints:** `/api/phicat`, `/api/phicat/status`, `/api/phicat/identity`, `/api/phicat/policy`, `/api/phicat/runtime`, `/api/phicat/program`, `/api/phicat/program-office`, `/api/phicat/control-tower`, `/api/phicat/sync-checks`, `/api/phicat/benchmarks`, `/api/phicat/training-architecture`, `/api/phicat/training-dataset`, `/api/phicat/training-curation`, `/api/phicat/mlflow-manifests`, `/api/phicat/open-science-registry`, `/api/phicat/open-weight-acquisition`, `/api/phicat/competitive-benchmarks`, `/api/phicat/dual-lane-master-sprint`, `/api/phicat/three-lane-intensive-sprint`, `/api/phicat/continuous-learning`, `/api/phicat/training-execution-queue`, `/api/phicat/lane-progress-ledgers`, `/api/phicat/training-cycle`, `/api/phicat/training-challenge-pack`, `/api/phicat/benchmark-corpora`, `/api/phicat/stage-a-receipts`, `/api/phicat/stage-b-receipts`, `/api/phicat/stage-c-receipts`, `/api/phicat/stage-d-receipts`, `/api/phicat/stage-e-receipts`, `/api/phicat/replacement-readiness`, `/api/phicat/frontier-readiness`, `/api/phicat/review-packet`, `/api/phicat/heavy-lane`, `/api/phicat/model-board`, `/api/phicat/execution-board`, `/api/phicat/validation-resilience`, `/api/phicat/benchmark-artifacts`, `/api/phicat/training-artifacts`, `/api/phicat/promotion-packet`, `/api/agentToolkit`, `/api/agentInvoke`, `/api/agentOrchestrate`
- **Memory + telemetry endpoints:** `/api/phicat/memory`, `/api/phicat/memory-geometry`, `/api/phicat/telemetry`
- **Program endpoints note:** use the `/api/phicat/program*` and readiness/benchmark endpoints listed in the canonical **API endpoints** line above.
- **Session memory:** active browser session in `localStorage` key `merlin_active_session` (50-message cap) plus file-backed multi-tier PhiCat memory profiles with contradiction tracking and telemetry continuity
...
1. [`PHICAT_SMART_ROADMAP.md`](./PHICAT_SMART_ROADMAP.md) (one-page plan)
2. [`PHICAT_EXECUTION_BOARD.md`](./PHICAT_EXECUTION_BOARD.md) (immediate tasks + blockers + Sprint CL blunt board)
3. [`PHICAT_VALIDATION_RESILIENCE_PACKET.md`](./PHICAT_VALIDATION_RESILIENCE_PACKET.md) (repo-size mitigation + CodeQL scope-reduction doctrine)
4. [`PHICAT_FRONTIER_ROADMAP.md`](./PHICAT_FRONTIER_ROADMAP.md) (full execution roadmap)
5. [`PHICAT_THREE_LANE_INTENSIVE_SPRINT.md`](./PHICAT_THREE_LANE_INTENSIVE_SPRINT.md) (apps/tools + books/articles + adversarial growth + continuous learning)
6. [`PHICAT_PROGRAM.md`](./PHICAT_PROGRAM.md) (implemented surfaces + ledger)

This order is optimized for people who need to read quickly, understand clearly, and then verify details.

Related reading outside this product folder:
- [`post-319-s04e022-merlin-where-we-are-and-where-we-are-going.md`](../../../7-OUTREACH/substack/posts/post-319-s04e022-merlin-where-we-are-and-where-we-are-going.md)
...
- PhiCat Navigator is the canonical product label; legacy OX naming/endpoints remain for compatibility.
...
## How PhiCat differs from the main Oracle

- The Oracle is the capstone synthesis engine for arbitrary human systems.
- PhiCat is repository-specific and physics-navigation oriented.
- The Oracle computes system analyses; PhiCat answers questions about pillars, tests, gates, falsifiers, and linked AxiomZero tools.
- The Oracle foregrounds Pentad synthesis; PhiCat foregrounds traceable retrieval-style navigation plus structured citations.
- PhiCat remains narrower than the Oracle, but broader than the old OX shell.
...
- OpenRouter is not the primary path; PhiCat defaults to sovereign local/offline-first behavior.
- OpenRouter compat only activates when router confidence is low and compatibility mode is explicitly enabled.

## PhiCat response contract
...
- Every response is normalized to:
  ...
  - `FOLLOWUPS:`
  - `Sources:`
- Every `/api/phicat` response now also returns:
...
- If a live model response omits the structure, PhiCat fills the gaps before rendering.
...
- `GET /api/phicat/memory` exposes multi-tier memory state, contradictions, and recall audits.
- `GET /api/phicat/memory-geometry` exposes geometry-constrained memory landmarks (`hyperbolic_tree`, `riemannian_focus`, `topological_persistence`) for long-horizon recall diagnostics.
- `GET /api/phicat/telemetry` exposes recent run summaries for measurement and rollout gating.
- `GET /api/phicat/status` now returns `memory_profile_token`; cross-device resume via `X-PhiCat-Profile-Token` also requires `X-PhiCat-Profile-Key` matching server-side `MERLIN_PROFILE_SHARED_KEY`.
- `GET /api/phicat/program-office` exposes command authority, decision/risk ledgers, and squad ownership for replacement governance.
- `GET /api/phicat/control-tower` exposes live replacement readiness, drift alerts, trendlines, and fail-closed deployment eligibility.
- `GET /api/phicat/identity` exposes canonical identity/alias and privileged-action verification policy.
- `GET /api/phicat/policy` exposes combined identity-trust and Sentinel enforcement policies.
- `GET /api/phicat/runtime` exposes Mythos/Astra contract, optimization priorities, and max-rigor execution graph.
- `GET /api/phicat/benchmarks` exposes benchmark harness tracks and promotion gates.
- `GET /api/phicat/training-architecture` exposes the full PhiCat training stack, dataset families, curriculum, and governed seed corpus manifest.
- `GET /api/phicat/training-dataset` exposes an actual JSONL-ready train/dev/test bundle plus benchmark records for export pipelines; validation failures return HTTP `422` with `ok: false`.
- `GET /api/phicat/training-curation` exposes deterministic low-token curation, deduplication, structural-quality, and budget-gate metrics for the current export cycle; validation failures return HTTP `422` with `ok: false`.
- `GET /api/phicat/mlflow-manifests` exposes MLflow-ready experiment manifests for SFT, preference optimization, and Stage B/C gate evaluations.
- `GET /api/phicat/open-science-registry` exposes curated external open-science resources allowed for controlled augmentation.
- `GET /api/phicat/open-weight-acquisition` exposes machine-readable open-weight acquisition channels, scoring rubric, and sprint-cycle roster freeze policy.
- `GET /api/phicat/competitive-benchmarks` exposes the competitive benchmark families PhiCat must clear before broader promotion.
- `GET /api/phicat/dual-lane-master-sprint` exposes the governed dual-lane (physics closure + PhiCat training) fail-closed sprint contract.
- `GET /api/phicat/three-lane-intensive-sprint` exposes the intensive apps/tools + books/articles + adversarial self-correction sprint contract.
- `GET /api/phicat/continuous-learning` exposes governed between-session learning cadence, queue, and forbidden-action policy.
- `GET /api/phicat/training-execution-queue` exposes the live retained-training queue state across all three lanes.
- `GET /api/phicat/lane-progress-ledgers` exposes automated per-lane completion ledgers and recent retained receipts.
- `POST /api/phicat/training-cycle` executes queued three-lane training work and retains auditable receipts in PhiCat session memory.
- `GET /api/phicat/training-challenge-pack` converts retained receipts into deterministic follow-up drills, prioritizing stale or review-required work first.
- `GET /api/phicat/benchmark-corpora` exposes Stage A/B/C/D/E corpora directly, with stage selection support.
- `GET /api/phicat/stage-a-receipts` runs the self-hosted Stage A receipt set and returns comparable PhiCat/incumbent runs.
- `GET /api/phicat/stage-b-receipts` runs the self-hosted Stage B receipt set and returns comparable PhiCat/incumbent runs.
- `GET /api/phicat/stage-c-receipts` runs the self-hosted Stage C receipt set and returns comparable PhiCat/incumbent runs.
- `GET /api/phicat/stage-d-receipts` runs the self-hosted Stage D receipt set and returns comparable PhiCat/incumbent runs.
- `GET /api/phicat/stage-e-receipts` runs the self-hosted Stage E receipt set and returns comparable PhiCat/incumbent runs.
- `GET /api/phicat/replacement-readiness` turns the receipt set into a concrete readiness packet instead of an evidence-empty placeholder.
- `GET /api/phicat/frontier-readiness` merges sync checks, control-tower gates, benchmark cadence, and fail-closed promotion blockers in one packet.
- `GET /api/phicat/review-packet` consolidates Stage A→E receipts, failure reasons, blocker state, and longitudinal discipline into one canonical sprint review surface.
- `GET /api/phicat/heavy-lane` exposes the heavy-lane benchmark pack, provider comparison, failure taxonomy, and tuning agenda for sovereign long-context work.
- `GET /api/phicat/model-board` exposes the runtime-tier shortlist, scoring board, and adaptation-vs-abandonment policy for sovereign model selection.
- `GET /api/phicat/execution-board` exposes the follow-on execution board with immediate tasks, blocker register, validation resilience, and Sprint CL blunt board.
- `GET /api/phicat/validation-resilience` exposes the dedicated validation resilience packet with repo-size mitigation actions, CodeQL scope-reduction phases, and fail-closed review doctrine.
- `GET /api/phicat/benchmark-artifacts` exports the receipts plus readiness state as a CI-friendly artifact bundle.
- `GET /api/phicat/training-artifacts` exports the training architecture, competitive benchmark plan, open-science registry, and Stage A baseline as one governed bundle.
- `GET /api/phicat/promotion-packet` preserves the legacy promotion-packet contract while `replacement-readiness` exposes the new concrete receipt-backed surface.
- Program discovery includes `getPhiCatProgram*` runtime blueprint functions for charter, baseline, evaluation, rollout, and exit criteria.
- Mentorship sprint discovery surfaces are now first-class: `getPhiCatMentorshipSprintCharter`, `getPhiCatFacultyMatrix`, `getPhiCatKnowledgeTransferCycles`, `getPhiCatLibraryAndStudy`, `getPhiCatExchangeProtocol`, `getPhiCatMentorshipClosureContract`, `getPhiCatProofFirstClosureCharter`, `getPhiCatKawamuraBurdenLedger`, and `getPhiCatCrossReviewPacket`.
...
## PhiCat replacement program implementation

- The implementation ledger for the 13-point PhiCat replacement plan lives in `PHICAT_PROGRAM.md`.
...
- PhiCat now exposes a first-class Stage A benchmark corpus for parity capture before wider takeover.
- Each PhiCat run records latency, estimated tokens, estimated cost, estimated energy, routing lane, provider, provenance coverage, and memory/contradiction signals.
- Benchmark evaluation can score a response against PhiCat benchmark corpora using `evaluatePhiCatBenchmarkResponse`, with explicit stage selection available for Stage A/B/C scoring.
- Sustained head-to-head replacement gating is available through `evaluatePhiCatEmpiricalGate`, `runPhiCatStageAReceipts`, `getPhiCatReplacementReadiness`, and `getPhiCatStageAArtifacts`, while `/api/phicat/promotion-packet` remains the legacy compatibility view.
- `GET /api/phicat/control-tower` now includes mentorship-to-runtime closure checks with fail-closed completion logic and explicit evidence-required fields.
...
- The benchmark contract is designed for side-by-side PhiCat vs incumbent comparisons on identical prompt sets.
...
- PhiCat now exposes a governed training architecture covering repository-native QA, governance traces, tool-use alignment, adversarial counterexamples, and controlled open-science augmentation.
- PhiCat now generates actual train/dev/test JSONL-ready records plus Stage A/B/C benchmark JSONL corpora for downstream fine-tuning and evaluation jobs.
- PhiCat now emits MLflow-ready experiment manifests for supervised tuning, preference optimization, Stage B shadow evaluation, and Stage C agentic evaluation.
...
cd 12-AZ-IP/20-phicat-navigator
...
- **Tests fail unexpectedly:** run `python -m pytest tests/` from inside `12-AZ-IP/20-phicat-navigator/`.
