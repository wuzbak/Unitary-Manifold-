...
**The Unitary Manifold v29.0 — Unambiguous Record**  
*This is the document a PRL referee should read first.*
*Last updated: 2026-09-01 (v29.0 — Sprint BE: Pillars 919–930; CKM 13D Yukawa texture audit, α_s 13D non-perturbative bound, second-CY₄ N_gen audit, F-theory Rung 10 partial closure, DESI DR3 live monitor update, PMNS ordering NLO stability, KK-tower ISW closure, Lean4 +120 (3276→3396); ~61,261 passed · 45 skipped · 12 deselected · 0 failed; next slot 931.)*
*Latest verified full regression: 61,261 passed · 45 skipped · 12 deselected · 0 failed (tests/ + recycling/ + Pentad; 2026-09-01).*
...
|-------------|----------------|-------------------|
| First-principles c_L (Yukawa texture) | **FITTED (partially constrained) — see note** | `src/core/yukawa_orbifold_bc_texture.py` derives the *pattern* c_L^{(n)} = ½ + (n_w−n)/(2n_w) from Z₂-even LH orbifold BCs, giving correct mass hierarchies. However, the actual c_L values used in `universal_yukawa.py` are computed by root-finding (bisection) against known experimental masses — i.e., they are fitted inputs, not independent predictions. Pillar 677 (<1.3% agreement) makes progress but still requires experimental mass input for calibration. Status: FITTED for the Yukawa sector as currently implemented. The orbifold-BC pattern is CONSTRAINED. Closing this fully requires deriving exact c_L values from geometry without observed masses as input. |
