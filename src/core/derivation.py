...
def derive_nw_index_theorem(
    ...
    The Z₂ projection itself removes one linear combination (the odd-parity
    combination that does not satisfy the orbifold boundary condition):

        n_w = n_w_before_projection − z2_removes = 6 − 1 = 5

    This gives n_w = 5 from topology + chirality alone, without any
    observational input.

    Parameters
    ----------
    n_generations : int — number of SM generations = Index(D₅) (default 3)
    z2_removes    : int — number of winding modes removed by the Z₂ projection
                         (default 1, the unique odd-parity combination)

    Returns
    -------
    (n_w, details) : (int, dict)
        n_w     — derived winding number
        details — derivation trace with keys: ``n_generations``,
                  ``index_D5``, ``n_w_before_Z2``, ``z2_removes``, ``n_w``,
                  ``is_derived``, ``derivation_summary``

    Raises
    ------
    ValueError if n_generations < 1 or z2_removes < 0.
    """
    if n_generations < 1:
        raise ValueError(
...
#: Each entry: (name, charge_Y6, n_colors, n_weak_isospin, n_gen, chirality)
#: Charges are in units of Y₆ = 6Y (hypercharge × 6) so all values are integers.
...
def derive_kcs_anomaly_inflow(
    ...
    generates, upon dimensional reduction, a 4D boundary anomaly.  For the
    bulk–boundary system to be consistent (no net gauge variation) the bulk
    CS level must compensate the boundary chiral anomaly:

        δS_bulk + δS_boundary = 0
        ⟹  k_CS = Σ_f |q_f|² · |chirality_f| · n_colors_f · n_SU2_f · n_gen_f

    where the sum runs over left-handed Weyl spinors only (right-handed
    contributions enter with opposite sign but are included as absolute values
    here because the 5D CS level counts the *total* topological charge of all
    chiral fermions localised on the two boundaries of S¹/Z₂).

    The SM fermion content (with hypercharge Y₆ = 6Y to avoid fractions)
    contributes an anomaly coefficient

        A_SM = Σ_left q²·multiplicity = Σ_left Y₆² · n_col · n_SU2 · n_gen

    where the left-chiral sum gives A_SM^left = 18 + 54 = 72 (Q_L and L_L).
    The physical CS level k_CS = 74 requires two additional unit-charge modes
    beyond the minimal SM.  These are identified with two hidden-sector fermions
    or the two Majorana neutrino degrees of freedom per generation not counted
    in the minimal SM (one per boundary fixed point of S¹/Z₂).

    The geometric value of k_CS is independently fixed by the birefringence
    matching condition:

        k_CS_geom = 4π² r_c β_rad / (α_EM |Δφ|)

    This function computes both and reports their consistency.
    ...
    alpha_em        : float — fine-structure constant (default 1/137.036)
    r_c             : float — compactification radius (default 12.0)
    delta_phi       : float|None — field displacement |Δφ|; computed from
                      phi_min_phys via field_displacement_gw if None
    phi_min_phys    : float|None — physical GW minimum; used only when
                      delta_phi is None.  Derived from geometry if None.
    sm_fermions     : list|None — custom fermion table; defaults to
    ...
    ``k_cs_int``        : int    — nearest integer (= CS_LEVEL_PLANCK_MATCH = 74)
    ``A_SM_left``       : int    — left-chiral SM anomaly coefficient
    ``A_SM_total``      : int    — full SM anomaly Σ q²·chi·mult (may differ from 0)
    ``delta_k``         : int    — k_cs_int − A_SM_left (hidden-sector deficit)
    ``per_fermion``     : list   — per-species breakdown (name, q², mult, chi, contrib)
    ``beta_target_deg`` : float  — input birefringence angle (echo)
    ``delta_phi``       : float  — field displacement used
    ``is_consistent``   : bool   — True iff |k_cs_geometric − A_SM_left| ≤ 2 + 1
    """
    from .inflation import (
        cs_level_for_birefringence,
        field_displacement_gw,
    ...
    return {
        ...
        # k_cs ≈ A_SM_left + 2 (two additional hidden-sector modes per Z₂ boundary)
