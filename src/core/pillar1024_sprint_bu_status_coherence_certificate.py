...
"""Pillar 1024 — Sprint BU status-coherence certificate."""
...
from src.core.pillar1023_sprint_bu_exact_kernel_certificate import pillar1023_summary
...
PILLAR_NUMBER: int = 1024
PILLAR_GATE: str = "SPRINT_BU_STATUS_COHERENCE_CERTIFICATE"
PILLAR_STATUS: str = "SPRINT_BU_STATUS_COHERENCE_CERTIFICATE_COMPLETE"
...
def status_surface_audit() -> Dict[str, Any]:
    """Audit that canonical surfaces narrate one Sprint BU story."""
    per_surface: Dict[str, Dict[str, Any]] = {}
    for name, path in STATUS_SURFACES.items():
        text = path.read_text(encoding="utf-8") if path.exists() else ""
        text_lower = text.lower()
        pillar_window_pass = bool(re.search(r"1021\s*[–-]\s*1024", text))
        next_slot_pass = (
            ("next slot 1025" in text_lower)
            or ("next_pillar_slot: 1025" in text)
            or ("next pillar slot" in text_lower and "1025" in text_lower)
        )
        open_hits = {label: (label in text) for label in REQUIRED_OPEN_LABELS}
        sprint_hits = {
            "v35.1": ("v35.1" in text),
            "pillar_window_1021_to_1024": pillar_window_pass,
            "next_slot_1025": next_slot_pass,
        }
        per_surface[name] = {
            "path": str(path),
            "exists": path.exists(),
    ...
    sprint_marker_surface_counts = {
        ...
        for key in ("v35.1", "pillar_window_1021_to_1024", "next_slot_1025")
    }
...
def status_coherence_certificate() -> Dict[str, Any]:
    """Return the Sprint BU status-coherence certificate."""
    audit = status_surface_audit()
    sprint = pillar1023_summary()
    valid = bool(
        audit["all_exist"]
    ...
    return {
        ...
        "interpretation": (
            "Sprint BU exact-kernel promotions and containment rules are synchronized "
            "across executable artifacts and canonical truth surfaces."
        ),
    }
...
def pillar1024_summary() -> Dict[str, Any]:
    """Return concise Pillar 1024 summary."""
    report = status_coherence_certificate()
    return {
        "pillar": PILLAR_NUMBER,
        "title": "Sprint BU Status-Coherence Certificate",
        "gate": PILLAR_GATE,
        "status": PILLAR_STATUS,
        "valid": PILLAR_VALID,
