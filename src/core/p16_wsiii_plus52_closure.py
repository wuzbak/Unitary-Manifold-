...
from src.sixd.solar_splitting_6dplus import K_CS, N_W, PI_KR, splitting_ratio_geometric
...
def p16_wsiii_gate_report() -> Dict[str, object]:
    ...
    gate1_nominal = fc_report["corrected_ratio"] > 0.0
    gate2_robust = abs(fc_report["f_c"] - FC_DERIVED) < 1e-15
    ...
    return {
        ...
        "fc_report": fc_report,
        "gates": {
            "gate1_nominal_positive": gate1_nominal,
            "gate2_exact_internal_consistency": gate2_robust,
            "gate3_axiomzero_plus52_derived": gate3_axiomzero,
        },
        "all_gates_pass": all_pass,
        "verdict": (
            "WS-III closure complete: +52 derived from T²/Z₃ moduli structure; "
            "P16 promotion gate unlocked."
            if all_pass
