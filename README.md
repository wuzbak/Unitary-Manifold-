...
> **Canonical status marker:** v37.7 Sprint CU · latest verified full regression in branch history: 64,150 passed · 22 skipped · 18 deselected · 0 failed.
...
**The foundational proposal concerns irreversibility.** The monograph proposes a geometric account of the arrow of time in a 5D parent structure. The repository explores that proposal through mathematical arguments and numerical implementations; full action-to-evolution equivalence and the physical interpretation of the flow remain obligations, not consequences of passing regression tests.
...
**Epistemic status:** Not all derivation chains are established. The [foundation reassessment](docs/TRUTH_LAYER.md#foundation-reassessment) corrects the metric normalization, withdraws the regular odd-composite photon argument, and distinguishes genuine calculations from assigned residual reductions and arithmetic proof proxies. Sprint CI further isolates the conditional KK block parameterization and smooth-circle tree-level coupling bookkeeping, leaving photon origin, action-to-evolution equivalence, independent CMB normalization, and joint UV predictivity explicit and open. Sprint CU now locks the next full-focus physics sprint to the single highest-leverage unresolved line — action-to-evolution — while recording PsiCat as benchmark-ready and eligible only for the next governed phase-2 applied-pressure promotion step under continued receipt discipline. These additions sharpen blocker precision and routing discipline without claiming direct Lean-term runtime execution or new hardgate closure; the present bridge remains manual port with traceability. The proposed CMB birefringence values β ∈ {≈0.273°, ≈0.331°} remain observationally testable: a value outside [0.22°, 0.38°], or in [0.29°, 0.31°], falsifies the braided-winding mechanism under its stated prediction. Gaps and tensions are tracked in [`FALLIBILITY.md`](FALLIBILITY.md) and [`1-THEORY/DERIVATION_STATUS.md`](1-THEORY/DERIVATION_STATUS.md). The Yukawa sector's mass-calibrated root-finding is not a top-down geometric derivation.
...
[![🌐 axiomzerospc.org](https://img.shields.io/badge/%F0%9F%8C%90-axiomzerospc.org-4a90e2)](https://axiomzerospc.org)
...
> **Current status (live):** Framework is internally self-consistent.
> Canonical live status and latest verified regression totals are tracked in [`STATUS.md`](STATUS.md), [`docs/mas_tracker.yml`](docs/mas_tracker.yml), and [`9-INFRASTRUCTURE/um_live_status.json`](9-INFRASTRUCTURE/um_live_status.json).
...
This repository's critical foundation is the original irreversibility programme:

1. [`6-MONOGRAPH/THEBOOKV9a (1).pdf`](<6-MONOGRAPH/THEBOOKV9a (1).pdf>) — the original monograph that set the programme everything else was built to prove
2. [`1-THEORY/UNIFICATION_PROOF.md`](1-THEORY/UNIFICATION_PROOF.md) — the explicit proof path from the 5D geometry to the larger framework
3. [`FALLIBILITY.md`](FALLIBILITY.md) — the hard boundaries, non-claims, and unresolved gaps

The later expansion layers (`4-IMPLICATIONS/`, `5-GOVERNANCE/`, `7-OUTREACH/`, and subsequent pillar packages) are downstream of that closure work; they are not the starting point for evaluating the foundation.
...
> |----------|-------------|
> | **[COMPLETION_REPORT.md](1-THEORY/COMPLETION_REPORT.md)** | **v9.33** — unsparing honest assessment of TOE status; 167 pillars + Ω₀ (SM 26-parameter ledger: 8 derived, 9 parameterized, 1 OPEN (Λ_QCD ×10⁷), Wolfenstein CKM, Higgs mass FTUM, solar mixing correction, Higgs VEV exact, neutrino lightest mass constraint, Newton G_N RS, CKM ρ̄ closure, Holon Zero certificate, Grand Synthesis Arc, Gap Closure Arcs I–II), open gaps (Λ_QCD ×10⁷, lightest ν UV condition, sin²θ₁₂ 13%, DE w₀ tension), path to experimental confirmation |
...
> of content, and all ~23,524 tests are passing in all three — but "passing" means
...
| 133 | CKM CP Sub-Leading Closure — δ_sub = 2·arctan(5/7) ≈ 71.08° (0.99σ from PDG 68.5°); braid opening angle derivation | `src/core/ckm_cp_subleading.py` | 63 |
...
| 141 | Newton's Constant from RS — G_N from RS1 self-consistency; M_KK ≈ 1041.8 GeV derived; CONSTRAINED (M₅ is UV seed) | `src/core/newton_constant_rs.py` | 41 |
| 142 | CKM Wolfenstein ρ̄ Closure — ρ̄_sub ≈ 0.119 (~25% from PDG 0.159); braid geometry + subleading phase | `src/core/ckm_rho_bar_closure.py` | 42 |
| Ω₀ | Holon Zero Certificate — irreducible geometric seed (n_w, k_CS, πkR, φ₀) → all 26 SM params; **Λ_QCD OPEN (×10⁷); 9 fermion masses PARAMETERIZED**; 8 DERIVED, 4 CONSTRAINED, 3 GEOMETRIC_ESTIMATE; living closure certificate | `src/core/holon_zero.py` | 71 |
| 143 | R-matrix Braid Neutrino — c_R = 23/25 THEOREM from orbifold fixed-point; 730× gap diagnosed | `src/core/rmatrix_braid_neutrino.py` | 45 |
| 144 | Neutrino RGE Bridge — RS zero-mode RGE; c_L = 0.961 numerical | `src/core/neutrino_rge_bridge.py` | 52 |
| 145 | Jarlskog Geometric — J ≠ 0 proved from n₁ ≠ n₂; CP violation geometric origin | `src/core/jarlskog_geometric.py` | 48 |
| 146 | Neutrino c_L UV Resolution — seesaw viable; c_L UV condition documented | `src/core/neutrino_cl_uv_resolution.py` | 51 |
| 147 | KK DE Radion Sector — radion ELIMINATED as DE candidate (fifth-force); w₀ tension open | `src/core/kk_de_radion_sector.py` | 53 |
| 148 | Non-Abelian Orbifold Emergence — SU(3)_C×SU(2)_L×U(1)_Y from n_w=5→SU(5)→Kawamura Z₂ | `src/core/non_abelian_orbifold_emergence.py` | 89 |
| 149 | CMB Acoustic Amplitude RG — suppression ×4.2–×6.1 quantified; open | `src/core/cmb_acoustic_amplitude_rg.py` | 97 |
| 150 | Neutrino Majorana UV Proof — Majorana seesaw from KK UV brane | `src/core/neutrino_majorana_uv_proof.py` | 82 |
| 151 | DE Equation of State DESI — w₀ = −0.9302 vs DESI/Planck; tension open | `src/core/de_equation_of_state_desi.py` | 79 |
| 152 | CMB Baryon-Photon R_b — R_b = 3ρ_b/(4ρ_γ) from KK baryon sector | `src/core/cmb_baryon_photon_rb.py` | 71 |
| 153 | Λ_QCD GUT RGE — α_s running M_Z→M_GUT; Λ_QCD ×10⁷ gap documented | `src/core/lambda_qcd_gut_rge.py` | 86 |
| 154 | Chiral Fermion Orbifold — chiral fermions from S¹/Z₂ orbifold fixed points | `src/core/chiral_fermion_orbifold.py` | 86 |
| 155 | KK DE wₐ CPL — wₐ = 0 (frozen radion); 2.1σ DESI tension open | `src/core/kk_de_wa_cpl.py` | 88 |
| 156 | Inflation as 5D — A_s RS inflation; F_RS ≥ 1 (RS enhances A_s; CMB suppression open) | `src/core/inflation_as_5d.py` | 79 |
| 157 | Neutrino Dirac Branch C — Branch C viable but disfavoured; c_R fine-tuning ~1% | `src/core/neutrino_dirac_branch_c.py` | 103 |
| 158 | *(Wave B reserved)* | — | — |
| 159 | Neutrino Mass Seesaw Canonical — 3-orders inconsistency resolved; Majorana seesaw | `src/core/neutrino_mass_seesaw_canonical.py` | 52 |
| 160 | KK Axion Quintessence — no wₐ ≠ 0 mechanism; DE as secondary falsification target | `src/core/kk_axion_quintessence.py` | 52 |
| 161 | Inflaton 5D Sector — A_s = UV-brane α parameter (~4×10⁻¹⁰); RS correction in wrong direction | `src/core/inflaton_5d_sector.py` | 52 |
| 162 | QCD Confinement Geometric (AdS/QCD) — m_ρ ≈ 0.760 GeV (2% from PDG); Λ_QCD ≈ 198 MeV (factor 1.7) | `src/core/qcd_confinement_geometric.py` | 101 |
| 163 | PMNS Solar Angle RGE — 1-loop RGE Δ(sin²θ₁₂) ≈ 1.4×10⁻⁴ (negligible; 13% gap remains) | `src/core/pmns_solar_rge_correction.py` | 80 |
| 164 | c_L Topological Classification — c_L = 71/74 THEOREM (0.16% from numerical) | `src/core/cl_topological_classification.py` | 67 |
| 165 | A_s Casimir Vacuum Bound — naturalness ratio ~5–6; A_s naturally bounded at GUT scale | `src/core/casimir_as_naturalness.py` | 74 |
...
| 168–181 | Red-Team Arc: α_GUT honest status, RS₁ Laplacian spectrum, fermion masses PARAMETERIZED, symbolic metric bridge | `src/core/` | ~194 |
| 182 | SM-RGE-free Λ_QCD from (n_w, K_CS); k_CS=74 topological proof; GW demotion; radion audit | `src/core/omega_qcd_phase_a.py` | ~90 |
| 183 | Axiom A callable + CFL guard + Λ_QCD hierarchy (Audit Response Arc) | `src/core/` | ~170 |
| 184 | φ₀ non-brittle attractor: max sensitivity S<0.1 | `src/core/sensitivity_analysis.py` | ~80 |
| 185 | EP Guard — EW radion safe by Yukawa mass screening; α=1/√6 fixed | `src/core/equivalence_principle_guard.py` | ~70 |
| 186 | LHC KK Resonances — G_KK invisible at LHC; B_KK^(1)≈2.5 TeV open tension | `src/core/lhc_kk_resonances.py` | ~75 |
| 187 | LHC KK Resonances audit (v9.39) | `src/core/lhc_kk_resonances.py` | ~75 |
| 188 | CKM Scaffold Analysis — WHY δ derives but θ_ij don't | `src/core/ckm_scaffold_analysis.py` | 76 |
| 189-A | RGE Running — α_GUT_geo=N_c/K_CS=3/74; upward 1-loop | `src/core/rge_running.py` | ~60 |
| 189-B | Bulk Eigenvalues — warp correction, KK mass spectrum | `src/core/bulk_eigenvalues.py` | ~60 |
| 189-C | GW Stabilizer — radion-GW coupling | `src/core/gw_stabilizer.py` | ~60 |
| 189-D | Action Minimizer — topological cutoff proof | `src/core/action_minimizer.py` | ~60 |
| 190 | Neutrino Winding — (5,7) braid inverted as (7,5); M_R~M_Pl | `src/core/neutrino_winding.py` | ~95 |
| 191 | Sakharov UM Audit — all 3 Sakharov conditions satisfied; η_B~3.3e-11 | `src/core/sakharov_um_audit.py` | ~89 |
| 192 | Neutrino Symmetry — RHN mapped to NEB; ε_NEB≈0.57% | `src/core/neutrino_symmetry.py` | 162 |
| 193 | Josephson Resonance — f_braid=35/74×f_plasma | `src/core/josephson_resonance.py` | ~55 |
| 194 | Bulk Eigenvalues warp correction (v10.2) | `src/core/bulk_eigenvalues.py` | ~50 |
| 195 | Resonance Audit — PoR Shannon entropy, ξ_c=35/74 | `src/governance/resonance_audit.py` | ~50 |
| 196 | SLA Manifesto + 8 kill-switches | `SLA_MANIFESTO.md` | — |
| 197 | SEP Stress-Energy Audit — SEP at 10⁻¹⁵; 5D vacuum; Casimir 3-layer | `src/core/sep_stress_energy_audit.py` | ~60 |
| 198 | BMU Ghost Stability — ghost-free proof; APS η̄=½; Proca stability; 5D Lorentz | `src/core/bmu_ghost_stability.py` | ~55 |
| 199 | GW Polarization Constraints — GW250114; H₀ 5σ→3σ; S₈ 3σ→2σ | `src/core/gw_polarization_constraints.py` | ~52 |
| 200 | RGE Geometric — AxiomZero forward chain; α_s(M_EW_geo)≈0.030 | `src/core/pillar200_rge_geometric.py` | 103 |
| 201 | Higgs VEV Geometric — v_Higgs=M_KK×√3/7≈257.6 GeV (4.6% off PDG) | `src/core/pillar201_higgs_vev_geometric.py` | ~50 |
| 202 | m_p/m_e Lattice-Free — m_p/m_e=K_CS²/N_c≈1825.3 (0.59% from PDG) | `src/core/pillar202_mp_me_lattice_free.py` | ~50 |
| 203 | KK Metric Feedback — KK QCD scheme audit | `src/core/pillar203_kk_metric_feedback.py` | ~50 |
| 204 | Topological c_L — c_L=71/74 topological | `src/core/pillar204_topological_cl_phys.py` | ~50 |
| 205 | Generation Quantization — N_gen=3 from braid quantization | `src/core/pillar205_generation_quantization.py` | ~50 |
...
> 🔒 **Repository at 785+ pillars + Ω₀ — v22.11, 2026-08-20.**
> Hardgate core: 208 pillars. Adjacent tracks: Pillars 218–785. Test count: ~56,964 passing (internal self-consistency). Lean4 976 theorems.
> Historical arc: 208 pillars + Ω₀ (v10.4 Near Closure), 420+ pillars + Ω₀ (v10.5–v13.4), 630+ pillars + Ω₀ (v20.9), 687+ pillars + Ω₀ (v21.0-S), 771+ pillars + Ω₀ (v22.8: Sprint AH Gap 1 PROVED_BY_EXHAUSTION; SU5 PROVED_CONDITIONAL; SwamplandAxiom formalised), 785+ pillars + Ω₀ (v22.11: Reader/Image Generator/Visualization overhaul; Pillar 785 G4 TYPE_B_CANDIDATE confirmed).
...
### Run the test suite — 0 failures (illustrative historical snapshot: 57,927 passed · 47 skipped · 12 deselected; v22.11, 2026-08-20 — see the canonical status marker at the top of this file or `STATUS.md` for the current verified total)
...
```
# tests/ (fast suite — Pillars 1–167 + Ω₀ + sub-pillars):
...
# ... SM Closure Arc (Pillars 133–142 + Ω₀) ...
tests/test_ckm_cp_subleading.py           63 passed  ← Pillar 133: CKM CP subleading (0.99σ) [TIER 1]
...
>
> **The 12 deselected tests** are in `test_richardson_multitime.py`, marked `@pytest.mark.slow`, and excluded from the default run by `addopts = -m "not slow"` in `pytest.ini`. They verify O(dt²) temporal convergence via Richardson extrapolation. Run with `pytest tests/ -m slow`.
...
(0 failures required; see the canonical status marker at the top of this
file or `STATUS.md` for the current verified full-suite total, since the
historical 57,927-count snapshot above is dated 2026-08-20/v22.11 and has
...
| F-5 | GR limit (internal) | `pytest` (32993 pass · 393 skip · 12 deselected) | Any non-zero GR-limit residual |
...
Emergent Irreversibility (v10.4). Zenodo.
