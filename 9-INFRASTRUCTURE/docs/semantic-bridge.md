...
> **Version:** v9.29+ — 132 pillars + sub-pillars, 17,438 passing tests  
...
This document covers Pillars 1–5 (the five original core modules). The framework has since expanded to **132 pillars**. Key addition points for contributors:
...
| Ω | `omega/omega_synthesis.py` | `UniversalEngine.compute_all()` — all 132 pillars in one call |

For the full predicate map of all 132 pillars, query:
```python
